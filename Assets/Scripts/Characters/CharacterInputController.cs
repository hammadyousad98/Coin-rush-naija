...
public class CharacterInputController : MonoBehaviour
{
	...
	protected void Update ()
    {
        ...
        // Get references to the keyboard device
...
#else
        // Use touch input on mobile (New Input System conversion)
        ...
        if (touchscreen != null && touchscreen.touches.Count > 0)
        {
            ...
            if (m_IsSwiping)
            {
                // touch.position.ReadValue() gets current touch coordinates
                Vector2 diff = touch.position.ReadValue() - m_StartingTouch;

                // Put difference in Screen ratio, but using only width, so the ratio is the same on both axes
                diff = new Vector2(diff.x / Screen.width, diff.y / Screen.width);

                if (diff.magnitude > 0.01f) // swipe distance to trigger movement set to 1% of screen width
                {
            ...
            // Read the touch phase structure
            var phase = touch.phase.ReadValue();

            if (phase == UnityEngine.InputSystem.TouchPhase.Began)
            {
                m_StartingTouch = touch.position.ReadValue();
                m_IsSwiping = true;
            }
